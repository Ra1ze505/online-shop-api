...
class Cart(object):
    ...
    def add(self, product, quantity=1, update_quantity=False):
        ...
        if update_quantity:  # Проверка total_quantity <= product.stock
            if quantity + self.cart[product_slug]['quantity'] <= Product.objects.get(slug=product_slug).stock:
                self.cart[product_slug]['quantity'] = quantity
        elif quantity + self.cart[product_slug]['quantity'] <= Product.objects.get(slug=product_slug).stock:
            self.cart[product_slug]['quantity'] += quantity
        self.save()
